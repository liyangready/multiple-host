	...
	case "mac":
        ...
        output.startFirefox = function(cb) {
            ...
            exec(firefoxPath + " -CreateProfile ff_dev", function(err) {
                ...
                if (fs.existsSync("~/Library/Application Support/Firefox/Profiles/")) {
                    profilePath = "~/Library/Application Support/Firefox/Profiles/";
                }
                else if (fs.existsSync("~/Library/Mozilla/Firefox/Profiles/")) {
                    profilePath = "~/Library/Mozilla/Firefox/Profiles/";
                }
                var arr = fs.readdirSync(profilePath);
                var proxyStr = 'user_pref("network.proxy.http", "127.0.0.1 ");';
                arr.forEach(function(item){
                   if (/ff_dev/.test(item)) {
                       fs.appendFileSync(profilePath + item, proxStr)
                   }
                });
            });
            var port = localStorage.getItem("serverPort") || 9393;
