...
switch(output.platform) {
	...
	case "mac":
        ...
        break;
        output.setSystemProxy = function() {

            var localStorage = global.window.localStorage;
            var port = localStorage.getItem("serverPort") || 9393;
            var enableCommand = ['sudo networksetup -setwebproxy Ethernet 127.0.0.1 ' + port,
        ...
        output.disableSystemProxy = function() {
            var disableCommand = ['sudo networksetup -setwebproxystate Wi-Fi off',
                                  'sudo networksetup -ssetwebproxystate Ethernet off']
            async.eachSeries(disableCommand, function iterator(item, callback) {
                exec(item, callback);
            }, cb);
        }
	case "linux64":
