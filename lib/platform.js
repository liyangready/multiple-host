...
switch(output.platform) {
	...
	case "mac":
		...
		output.startChrome = function(cb) {
			...
			command ='open ' + chromePath + " --args '--proxy-server=http://127.0.0.1:" + port +   "' '--user-data-dir=" + devPath +  "' '--lang=local'  'http://wiki.corp.qunar.com/pages/viewpage.action?pageId=77931765' ";
            console.log(command);
            exec(command, cb);
		}
